...
def validate_config():
    ...
    required_vars = ["API_ID", "API_HASH", "BOT_TOKEN", "OWNER_ID", "MONGO_URL", "LOGGER_ID"]
    ...
    try:
        ...
        if logger_id > 0:  # Channel IDs should be negative
            raise ValueError("LOGGER_ID must be a negative integer (channel ID)")
    except ValueError:
        raise ValueError("LOGGER_ID must be a valid integer")
...
async def encode_link(link: str, client: Client, owner_id: int) -> str:
    ...
    try:
        ...
        print(f"Error in encode_link: {e}")
        raise
...
def generate_base64_string(msg_id: int) -> str:
    ...
    try:
        ...
        print(f"Error in generate_base64_string: {e}")
        raise
...
async def decode_link(encoded_str: str) -> int:
    ...
    try:
        ...
        print(f"Error in decode_link: {e}")
        raise
...
async def handle_start(client, message):
    try:
        ...
        try:
            ...
            print(f"Failed to log to logger channel: {e}")
            await client.send_message(OWNER_ID, f"⚠️ Failed to log to logger channel: {e}")
...
async def startup_notification():
    ...
    try:
        # Verify we can access both critical chats
        await app.get_chat(OWNER_ID)
        try:
            await app.get_chat(LOGGER_ID)
        except Exception as e:
            await app.send_message(OWNER_ID, f"⚠️ Warning: Cannot access logger channel: {e}")
        
        # Send startup notification
        await app.send_message(OWNER_ID, "🤖 Bot started successfully!")
    except Exception as e:
        print(f"⚠️ Critical startup error: {str(e)}")
        # Attempt to write to stderr as last resort
